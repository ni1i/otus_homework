...
**Контейнер** - это отдельный, изолированный процесс, который может быть запущен и остановлен и в котором исполняются сервисы.

**Образ** представляет собой слепок виртуальной машины с набором утилит и настроек, в нём могут быть запущены несколько контейнеров.

* Можно ли в контейнере собрать ядро:

Собрать ядро можно, но запустить его не получится, так как контейнер всегда использует ядро хостовой системы.
